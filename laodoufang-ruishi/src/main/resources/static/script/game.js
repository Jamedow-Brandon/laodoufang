var Game = function () {
    ...
    this.falledStat = {
        ...
        shouldJumpWidth: 0//当前应跳跃的距离
    };
    this.fallingStat = {
        speed: 0.3, // 游戏失败后垂直方向上的掉落速度
        end: false // 掉到地面没有
    };
}
Game.prototype = {
    ...
    _createCube: function () {
        var material = new THREE.MeshLambertMaterial({color: this.config.cubeColor});//材料
        var geometry = new THREE.CubeGeometry(this.config.cubeWidth, this.config.cubeHeight, this.config.cubeDeep);//几何模型
        var type = this._randomNum(0, 4);
        switch (type) {
            case 1:
                geometry = new THREE.CylinderGeometry(2, 2, 2, 100);
            ...
            case 2:
                ...
                break;
            case 3:
                geometry = new THREE.CylinderGeometry(1, 1, 2, 100);
                this.config.cubeWidth = 1;
                break;
            case 4:
                geometry = new THREE.CylinderGeometry(1, 2, 2, 100);
                this.config.cubeWidth = 1;
                break;
        }

        var mesh = new THREE.Mesh(geometry, material);//网孔
